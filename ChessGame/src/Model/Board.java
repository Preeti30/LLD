...
public class Board {
    ...
    private void intiaizeBoard() {
        ...
        for(int i=0;i<8;i++)
        {
            board[7][i] = new Pawn(Color.WHITE, 7, i);
        }
    }
    ...
    {
        if(piece==null || destRow<0 || destRow>7 || destCol>7 || destCol<0)
        {
            return false;
        }
    ...
    public boolean isCheckmate(Color color) {
        ...
        //now it will check if there is way for king to save him
        for(int row=0; row<8; row++)
        {
            for(int col=0; col<8; col++)
            {
                //Go through all current player’s pieces. Try moving each piece to every possible square
    ...
    public boolean isInCheck(Color color)//find the king of given color and check if any aopponent piece can come to king's place
    {
        int kingRow = -1;
        int kingCol = -1;
